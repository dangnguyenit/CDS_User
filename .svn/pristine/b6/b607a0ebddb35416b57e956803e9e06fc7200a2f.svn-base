...
class SessionsController < Devise::SessionsController
  ...
  def create
    ...
    decrement_failed_attempts(user) if recaptcha_present?(params) and !verify_recaptcha
  ...
  def adjust_failed_attempts(user)
    if user.failed_attempts > user.cached_failed_attempts
      user.update_attributes(cached_failed_attempts: user.failed_attempts)
    else
  ...
  def increment_failed_attempts(user)
    ...
    user.update_attributes(failed_attempts: user.cached_failed_attempts)
  end
